...
import Node.Buffer (class MutableBuffer, Buffer, STBuffer, BufferValueType(..), concat', copy, create, fill, freeze, fromArray, fromArrayBuffer, toArrayBuffer, fromString, getAtOffset, setAtOffset, read, readString, runST, thaw, toArray, toString, write, slice)
...
test = do
  ...
  testBuffer (Proxy :: Proxy Buffer) identity

  log "Testing Node.Buffer [ST] ..."
  testBuffer (Proxy :: Proxy (STBuffer _)) (unsafeCoerce ST.run >>> pure)
  log " - runST"
  testRunSt

testBuffer :: forall buf m. MutableBuffer buf m =>
...
testBuffer _ run = do
  ...
  where
    ...
    testFreeze = do
      buf <- Immutable.toArray <$> run (fromArray [1, 2, 3] >>= freeze)
      assertEqual {expected: [1, 2, 3], actual: buf}

    testThaw :: Effect Unit
    testThaw = do
      buf <- run (thaw (Immutable.fromArray [1, 2, 3]) >>= toArray)
      assertEqual {expected: [1, 2, 3], actual: buf}
...
testRunSt = do
  let buf = Immutable.toArray $ runST (create 3)
  assertEqual {expected: [0, 0, 0], actual: buf}
